exports.run = async (client, msg, args) => {
  ...
  if(member.id === "466288781918732308")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (':no_entry_sign: Ya ahaha sen kimsin yapımcıma laf söyleceksin düdük.')
}})
  if(member.id === "466288781918732308")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`:no_entry_sign: Hoop! bir şeyler ters gitti yapımcıma laf söylemek mi ahaha.!`)
}})
  if(member.id === "466288781918732308")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`:no_entry_sign: Hoop! Orda durucan Yapımcım Lokus'a Laf söylemezsin aslan.`)
}})
    else{
