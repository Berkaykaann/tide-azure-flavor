exports.run = async (client, msg, args) => {
 ...
 description: (':no_entry_sign: Sen Kimsin Sahibime Sövüyon Amına Kodumun Jedayı.')
  ...
  if(member.id === "466288781918732308")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`:no_entry_sign: Hoop! Birşeyler ters gitti Lokus  etiketlemişsin yarram!`)
}})
  if(member.id === "466288781918732308")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`:no_entry_sign: Hoop! Orda durucan Yapımcım Lokus'a sövemezsin amını sikerim.`)
}})
